...
from Network.GETANet import fusion_model
    ...
    def train_eopch(self):
        ...
        for step, (inputs, points, st_sizes) in enumerate(self.dataloaders['train']):
            ...
            with torch.set_grad_enabled(True):
                outputs,outputs_2,outputs_3,outputs_4 = self.model(inputs)
                prob_list = self.post_prob(points, st_sizes)
                loss_1 = self.criterion(prob_list, outputs)
                loss_2 = self.criterion(prob_list, outputs_2)
                loss_3 = self.criterion(prob_list, outputs_3)
                loss_4 = self.criterion(prob_list, outputs_4)
                loss = (loss_1+loss_2+loss_3+loss_4) / 4.0
